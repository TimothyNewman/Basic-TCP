...
class StudentUSocket(StudentUSocketBase):
  ...
  def connect(self, ip, port):
    ...
    ## Start of Stage 1 ##
    packet = self.new_packet(False, None, True)
    packet.tcp.seq = self.snd.iss
    self.tx(packet)
    self.state = SYN_SENT
    ## End of Stage 1 ##
  ...
  def tx(self, p, retxed=False):
    ...
    if (p.tcp.SYN or p.tcp.FIN or p.tcp.payload) and not retxed:
      ...
      self.snd.nxt += len(p.tcp.payload)
      pass
  ...
  def rx(self, p):
    ...
    * Send pending ACKs and FINs
    ...
    while not self.rx_queue.empty():
      smallest_packet = self.rx_queue.get_earliest_pkt()
      if smallest_packet[0] |GT| self.rcv.nxt:
        self.set_pending_ack()
        break
      pop_packet = (self.rx_queue.pop())[1]
      data = pop_packet.app[self.rcv.nxt | MINUS | pop_packet.tcp.seq:]
      self.handle_accepted_seg(pop_packet.tcp, data)
  ...
  def update_rto(self, acked_pkt):
    ...
    r_prime = self.stack.now - acked_pkt.tx_ts
    self.rttvar = (1 - self.beta) * self.rttvar + self.beta * abs(self.srtt - r_prime)
  ...
  def maybe_send(self):
    ...
    while remaining > 0:
      slice = min(self.mss, remaining)
      payload = self.tx_data[bytes_sent:bytes_sent+slice]
  ...
  def check_timer_retx(self):
    ...
    if not self.retx_queue.empty():
      packet_tuple = self.retx_queue.peek()
      time_in_queue = self.stack.now - packet_tuple[1].tx_ts
      if time_in_queue >= self.rto:
        self.tx(packet_tuple[1], retxed=True)
